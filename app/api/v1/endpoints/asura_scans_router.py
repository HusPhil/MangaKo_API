...
async def get_latest_manga(page: int) -> LatestMangaListResponse:
    url = f"https://asurascanz.com/manga/?page={page}&status=&type=&order=latest"
    try:
        scraper = get_scraper(source)
...
async def get_popular_manga() -> PopularMangaListResponse:
    url = f"https://asurascanz.com/manga/?page=1&status=&type=&order=popular"
    try:
        scraper = get_scraper(source)
