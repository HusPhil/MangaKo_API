...
async def get_latest_manga(page: int) -> LatestMangaListResponse:
    url = f"https://weebcentral.com/search/data?author=&text=&sort=Latest+Updates&order=Descending&official=Any&anime=Any&adult=Any&display_mode=Full%20Display"
    try:
        scraper = get_scraper(source)
...
async def get_popular_manga() -> PopularMangaListResponse:
    url = f"https://weebcentral.com/search/data?author=&text=&sort=Popularity&order=Descending&official=Any&anime=Any&adult=Any&display_mode=Full%20Display"
